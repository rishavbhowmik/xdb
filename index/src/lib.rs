...
mod tests {
    ...
    fn kv_map_to_bytes_test() {
        let mut kv_map: LinkedList<kv_tupple::KVTupple> = LinkedList::new();
        let kv_tupple = kv_tupple::KVTupple::new(&[0x10, 0x20, 0x30, 0x40], &[0x15, 0x25, 0x35, 0x45, 0x55, 0x65]);
        kv_map.push_back(kv_tupple);
        let kv_tupple = kv_tupple::KVTupple::new(&[0x10, 0x20, 0x30, 0x41], &[0x15, 0x25, 0x35, 0x45, 0x55, 0x66]);
        kv_map.push_back(kv_tupple);
    ...
    fn btree_index_to_bytes_test() {
        let mut btree_index: BTreeIndex = BTreeMap::new();
        btree_index.insert(vec![0x10, 0x20, 0x30, 0x40], vec![0x15, 0x25, 0x35, 0x45, 0x55, 0x65]);
        btree_index.insert(vec![0x10, 0x20, 0x30, 0x41], vec![0x15, 0x25, 0x35, 0x45, 0x55, 0x66]);
    ...
    fn hash_index_to_bytes_test() {
        let mut hash_map: HashIndex = HashMap::new();
        hash_map.insert(vec![0x10, 0x20, 0x30, 0x40], vec![0x15, 0x25, 0x35, 0x45, 0x55, 0x65]);
        hash_map.insert(vec![0x10, 0x20, 0x30, 0x41], vec![0x15, 0x25, 0x35, 0x45, 0x55, 0x66]);
