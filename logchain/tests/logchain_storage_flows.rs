...
fn make_segment_payload_list_new_storage() {
    ...
    assert_eq!(result.is_ok(), true);
    let (segment_list, first_block_index, last_block_index) = result.unwrap();
    // ([(0, [1, 0, 0, 0, 17, 18, 19, 20, 21, 22, 23, 24]), (1, [2, 0, 0, 0, 25, 26, 27, 28, 29, 30, 31, 32]), (2, [3, 0, 0, 0, 33, 34, 35, 36, 37, 38, 39, 40]), (3, [4, 0, 0, 0, 41, 42, 43, 44, 45, 46, 47, 48]), (4, [255, 255, 255, 255, 49, 50, 51, 52])], 0, 4)
    assert_eq!(first_block_index, 0);
    ...
    assert_eq!(
        segment_list[2].1.len(),
        SIZE_OF_BLOCK_INDEX + block_len as usize
    );
    assert_eq!(segment_list[3].0, 3);
    assert_eq!(
        segment_list[3].1.len(),
        SIZE_OF_BLOCK_INDEX + block_len as usize
    );
    assert_eq!(segment_list[4].0, 4);
    assert_eq!(
        segment_list[4].1.len(),
        SIZE_OF_BLOCK_INDEX + (log_1_data.len() % block_len as usize) as usize
    );
